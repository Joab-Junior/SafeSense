<?php
    ...
    $stmt = $conn->prepare("SELECT id_alerta, nivel_gas, st_alerta FROM alertas_gas WHERE id_usuario = ? ORDER BY dt_alerta DESC LIMIT 1");
    ...
    if ($row) {
        jsonResponse('success', 'Último alerta', $row);
    } else {
        jsonResponse('success', 'Nenhum alerta encontrado', null);
    }
?>